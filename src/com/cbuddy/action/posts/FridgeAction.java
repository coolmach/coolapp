...
public class FridgeAction extends ActionSupport implements SessionAware, ServletRequestAware, ModelDriven<FridgePostDetails>{

	private static final long serialVersionUID = 1L;

	FridgePostDetails postDetails = new FridgePostDetails();
	private File upload;
	...
	private List<FridgePostDetails> adList = new ArrayList<FridgePostDetails>();
	...
	private boolean validateMandatoryFields(){
		...
		if(postDetails.getDoubleDoor() == null || postDetails.getDoubleDoor().equals("")){
			addFieldError("errorMsg", "Please select the number of doors");
			return false;
		}
	...
	private boolean validateFieldLength(){
		...
		temp = postDetails.getColor();
		if(temp != null && temp.length() > 8){
			addFieldError("errorMsg", "Invalid Color");
			return false;
		}
		
		double capacity = postDetails.getCapacity();
		if(capacity > 1000){
			addFieldError("errorMsg", "Invalid Capacity in litres");
			return false;
		}
		
		temp = postDetails.getDoubleDoor();
		if(temp != null && temp.length() > 1){
			addFieldError("errorMsg", "Invalid Number of Doors");
			return false;
		}
	...
	public String postAd(){
		...
		poit.setSubCategory(CBuddyConstants.SUBCATEGORY_ELECTRONICS_AND_HOUSEHOLD_FRIDGE);
		...
		//Make an entry in PFridge
		PFridge entity = new PFridge();
		...
		entity.setColor(postDetails.getColor());
		entity.setCapacity(postDetails.getCapacity());
		entity.setDoubleDoor(postDetails.getDoubleDoor());
	...
	private void populateAdditionalDetailsForPost(FridgePostDetails postDetails, Session dbSession){
	...
	public String getAdListForCriteria(){
		...
		if(postDetails.getSubCategory()==null || postDetails.getSubCategory().equals("") || subCategoryStr.equals("")){
			postDetails.setSubCategory(CBuddyConstants.SUBCATEGORY_ELECTRONICS_AND_HOUSEHOLD_FRIDGE);
			subCategoryStr = Utils.getInstance().getSubCategoryDesc(postDetails.getCategory(), postDetails.getSubCategory());
		}
	...
	@Override
	public FridgePostDetails getModel() {
		return postDetails;
	}
	...
	public List<FridgePostDetails> getAdList() {
		return adList;
	}

	public void setAdList(List<FridgePostDetails> adList) {
		this.adList = adList;
	}
