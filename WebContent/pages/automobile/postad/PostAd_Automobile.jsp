...
<div class="row">
	...
	<div class="col-md-8" style="margin-left:-50px;">
		<s:form cssClass="form-horizontal post" id="postContentForm" action="automobile_postAd.action" enctype="multipart/form-data" onsubmit="return localValidations()">
			<input type="hidden" name="category" value="AUTO"/>
			<div class="col-md-12">
				<div class="form-group" >
					<label class="col-sm-10 col-sm-offset-3 postAdHeading">
					<span>Post Ad - Cars, Bikes, Cycles, etc.</span> <span style="font-size:12px;"><s:fielderror fieldName="errorMsg"/></span></label>
				</div>
				<div class="form-group">
					<label class="col-sm-3 control-label">Vehicle Type</label>
					<div class="col-sm-7" style="padding-top:6px;">
						<s:select class="dropDown" style="height:30px;width:80px;" id="itemType" name="itemType" theme="simple" onChange="displayPostAdSection()"
								headerKey="-1" headerValue="Select"
								list="#{'1':'Cars', '2':'Bikes', '3':'Cycles'}"
								value="subCategory" />
				...
				<div id="postContent">
					...
					<s:elseif test="subCategory == 3">
						<%@ include file="./PostAd_Automobile_Cycles.jsp" %>
					</s:elseif>
				</div>
			</div>
		</s:form>
...
<script>
	function displayPostAdSection(){
		...
		$.ajax({
			url: "Virat" + "/getDetailsSection_Automobile?",
	...
	function updateToolTip(subCategory){
		if(subCategory == "1"){$("#Title").attr("data-infoTip", "E.g. 'Maruti Alto 2010, excellent condition for immediate sale'");}
		else if(subCategory == "2"){$("#Title").attr("data-infoTip", "E.g. Honda Activa 2008 model available for sale.");}
		else if(subCategory == "3"){$("#Title").attr("data-infoTip", "E.g. Hero Cycle (without Gear), Good condition, available for sale.");}
	}
